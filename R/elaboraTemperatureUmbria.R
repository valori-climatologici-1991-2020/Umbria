...
sensore<-c("pluviometro","termometro")[2]
...
purrr::map(ffile,.f=~{
  ...
  read_delim(.,delim=";",col_names = TRUE,skip = 1,col_types = cols(.default = col_double()))->x;
  x$id<-idSensore
  x$yy<-yy
...
correggiNomi<-function(x){
  ...
  names(x)<-c("id","yy","dd",paste(rep(month.abb,each=3),c("Tmax","Tmin","Tmean"),sep="."))
...
purrr::walk(c("Tmax","Tmin"),.f=function(PARAM){
